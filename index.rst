...
.. code-block:: sh
   ...
   # Select ('UPN' is added) what columns we are interested in: 
   Select-object "UPN", "Serial Number", "Secret Key", "Time Interval", "Manufacturer", 
   "Model" |
   # Correct the sorting so that it is ascending order by authenticator serial number:
   Sort-Object "Serial Number" | 
   # Update make and model for customer visibility in the AAD portal:
   ForEach-Object {
   # Replace "GA" with "Thales":
   $_.Manufacturer = $_.Manufacturer -replace 'GA', 'Thales'
   # Also replace "LT" with "OTP110":
   ...
   [console]::beep(300, 150); Write-Host "`nOK, done! Your original file has been modified."
